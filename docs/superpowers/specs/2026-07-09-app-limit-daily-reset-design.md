...
Scope per-app usage state by both rule ID and usage date:

```
offset.<rule-id>.<yyyy-mm-dd>
reported.<rule-id>.<yyyy-mm-dd>
```
...
1. The monitor extension determines the sample's `usageDate`.

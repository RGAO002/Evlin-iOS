...
def gate_command(gate: str) -> tuple[Path, list[str] | str]:
    commands: dict[str, tuple[Path, str]] = {
        ...
        "debug-xctest-build": (ios, "DERIVED=\"$PWD/.superpowers/evidence/metering-phase3/DerivedData-DebugTests\"; if [ -e \"$DERIVED\" ]; then mv \"$DERIVED\" \"$DERIVED.previous-$(date +%s)-$$\"; fi; SENTRY_SKIP_DSYM_UPLOAD=1 xcodebuild -project 'Evlin iOS.xcodeproj' -scheme 'Evlin iOS' -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.3.1' -derivedDataPath \"$DERIVED\" CODE_SIGNING_ALLOWED=NO IPHONEOS_DEPLOYMENT_TARGET=17.6 TARGETED_DEVICE_FAMILY='1,2' build-for-testing"),
